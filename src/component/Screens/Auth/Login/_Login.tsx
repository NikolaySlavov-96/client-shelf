...
const _Login = () => {
  ...
  const handleMagicLink = useCallback(async () => {
    ...
    try {
      ...
      Toast({
        title: TEXTS.TOAST_MAGIC_LINK_SENT,
        typeIcon: ESwalIcon.SUCCESS,
      });
    } finally {
  ...
  const handleSubmit = useCallback(
    async (e: FormEvent) => {
      ...
      try {
        ...
        if (
          result?.messageCode === ServerError.SUCCESSFULLY_LOGIN.messageCode
        ) {
          Toast({
            title: TEXTS.TOAST_LOGIN_SUCCESS,
            typeIcon: ESwalIcon.SUCCESS,
          });
      ...
      <div className={`flex-start ${styles.right}`}>
        <div className={styles.card}>
          ...
          <AuthTabs
            activeTab="login"
            onSwitch={(tab) =>
              tab === "register" ? navigate(ROUT_NAMES.REGISTER) : undefined
            }
          />
