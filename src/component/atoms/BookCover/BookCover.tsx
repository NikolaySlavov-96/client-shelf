...
function BookCover({
  ...
  return (
    <div
      className={cx(styles.cover, styles[`cover--${variant}`], className)}
      style={{ background: gradient }}
    >
      {fileUrl ? (
        <img className={styles.img} src={fileUrl} alt={fileSrc ?? productTitle} />
      ) : null}

      {isDetail ? (
        !fileUrl ? <span className={styles.placeholder}>{productTitle}</span> : null
      ) : (
