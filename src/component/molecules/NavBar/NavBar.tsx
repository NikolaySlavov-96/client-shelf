...
function NavBar({ className }: INavBarProps) {
  ...
  const handleProfileClick = useCallback(
    () => navigate(ROUT_NAMES.USER_COLLECTION),
    [navigate],
  );
  ...
  return (
    <header className={cx(styles.nav, "flex-between", "sticky-top", className)}>
      ...
      >
        ...
        <span className={styles["nav__logo-accent"]}>
          {TEXTS.NAV_LOGO_SUFFIX}
        </span>
      </a>
      ...
      <nav className={linksClass} aria-label="Main navigation">
        {isAuthenticated ? (
          <>
            <button
              className={cx(styles.nav__user, "flex-align")}
            ...
            <Button
              label={TEXTS.NAV_COLLECTIONS}
              variant="ghost"
              size="md"
              onClick={handleProfileClick}
            />
            <Button
              label={TEXTS.NAV_LOGOUT}
              variant="ghost"
              size="md"
